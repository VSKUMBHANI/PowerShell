# Ref: https://github.com/MichaelGrafnetter/DSInternals
# Works within domain joined pc.
# AD must be accessible during executing the script.
# Must tick the option "Store passwords using reversible encryption" in the AD user's property.
# Note: "Store passwords using reversible encryption" is not safe as per the Microsoft guideline. Please refer to the link https://learn.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/store-passwords-using-reversible-encryption
...
$Cred = (Get-ADReplAccount -SamAccountName $User -Domain $Domain -Server $Server).SupplementalCredentials.ClearText
New-WPFMessageBox -Content "Password for $User is: $Cred" -Title "Password" -TitleFontSize 15 -ContentFontSize 15
